...
#define MIN_MOTOR -250
#define MOTOR_UNDERFLOW 75
...
int32_t pid_update(pid_controller *pid, int32_t target, int32_t meas) {
    ...
    //int32_t i = pid->integral >> 1 + fixedpt_mult(pid->ki, ((pid->prev_err + err) >> 1) >> TIMESTEP) >> 1;
    int32_t i = pid->integral >> 1 +  err >> TIMESTEP >> 1;
    pid->integral = i;
    pid->prev_err = err;
    ...
    if (motor_speed > MAX_MOTOR) {
      ...
      return MIN_MOTOR;
    } /*else if (motor_speed < MOTOR_UNDERFLOW && motor_speed > 0) {
      return MOTOR_UNDERFLOW;
    } else if (motor_speed < 0  && motor_speed > -MOTOR_UNDERFLOW) {
      return -MOTOR_UNDERFLOW;
    }*/ else {
      return motor_speed;
    }
